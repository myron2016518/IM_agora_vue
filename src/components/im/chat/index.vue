<template>
  <div class="userlist">
    ...
    >
      <a-menu-item
        style="height: 80px; position: relative; textAlign: left; borderBottom: 1px solid #eee; margin: 0;backgroundColor: #fff;"
        v-for="item in userList[type]"
        :key="getKey(item)"
        @click="select2(item, getKey(item))"
      >
        <a-row class="im_ulist_1">
          <a-col :span="4">
            <img
              v-if="item.logo_url"
              class="user_avatar"
          ...
          <a-col :span="18" class="im_ulist_1_2">
            <span class="custom-title">{{
              getUserName(item.imName || item.name)
            }}</span>
    ...
    <div v-if="userList.length == 0" class="userlist_nofirend">暂无好友</div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    console.log("===userList===", this.userList, this.type);
  },
  updated() {
    this.scollBottom();
  },
  ...
  methods: {
    ...
    getUserName(_txt) {
      let _nt = _txt;
      _nt.length > 10 && (_nt = _txt.slice(0, 15) + "...");
      return _nt;
    },
    ...
    select2(key, index) {
      ...
      //获取当前用户的消息列表
      this.$store.commit("updateSelectedUser", key);
    },
    loadMoreMsgs() {
      const me = this;
    ...
    getLastMsg(item) {
      const { name, params } = this.$route;
      const chatList = this.chatList[this.type];
