<template>
  <Draggable v-if="visible" id="drag2">
    <div class="call-wrapper PC">
      ...
      <!-- 多人页面 -->
      <div class="videos-wrapper multi" v-else v-show="pushedStream">
        <div class="item-wrapper" :class="computed_layout()">
          <div class="name">{{ user + "(我)" }}</div>
          <video ref="localVideo" autoPlay muted playsInline />
        </div>
        <div
      ...
      <div class="media-action-wrapper" v-if="pushedStream">
        ...
        ></i>
        <i
          class="el-icon-video-camera font camera"
  ...
  methods: {
    ...
    onStreamAdded(member, stream) {
      if (!stream.located()) {
        ...
        if (uid == this.$data.user) {
          // 收到相同用户发流
          console.log(
            "[Call Component]  hangup onStreamAdded uid == this.$data.user"
          );
          this.$message({
            message: "已在其他设备处理",
            type: "warning",
          ...
          // 修改 title 1v1
          if (this.$data.callType != 2 && this.$data.call_role != "callee") {
    ...
    onConfrAttrsUpdated(confr_attrs) {
      console.log(
        "[Call Component]  onConfrAttrsUpdated",
        JSON.stringify(confr_attrs)
      );

      // 区分是否是自己的 uid,以此判断 是主叫还是被叫
      let invitee_attrs = confr_attrs.filter(
        (item) => item.key.indexOf("invitee_") > -1 && item.op != "DEL"
      );

      let del_invitee_attrs = confr_attrs.filter(
        (item) => item.key.indexOf("invitee_") > -1 && item.op == "DEL"
      );

      if (invitee_attrs.length > 0)
        console.log(
          "[Call Component]  invitee_attrs",
          JSON.stringify(invitee_attrs)
        );
      if (del_invitee_attrs.length > 0)
        console.log(
          "[Call Component]  del_invitee_attrs",
          JSON.stringify(del_invitee_attrs)
        );
        ...
        if (uid) {
          if (uid == _this.$data.user) {
            ...
            _this.$data[`invitee_attr_timer_${uid}`] = setTimeout(() => {
              ...
              console.log(
                "[Call Component]  member invitee timeout",
              ...
              if (member && member.status == "waiting") {
                ...
                if (_this.$data.callType != 2)
                  _this.update_members(uid, "timeout"); // 1v1
              }
            }, 30000);
          }
        }
      ...
      del_invitee_attrs.map((item) => {
        ...
        if (uid == _this.$data.user) {
          ...
          if (_this.$data.call_status != "talking") {
            // 自己如果 talking, 则忽略(自己删除的会议属性)
            console.log(
              "[Call Component]  hangup del_invitee_attrs != talking"
            );

            _this.hangup();
          }
        } else {
          // 邀请的信息已处理或超时后的被删掉 会议属性
          console.log(
            "[Call Component]  invitee_attr_timer",
            _this.$data[`invitee_attr_timer_${uid}`]
          );
          ...
          let member = _this.$data.members[uid];
          console.log(
            "[Call Component]  del_invitee_attrs map member=" + uid,
            JSON.stringify(member)
          );
          ...
          if (member) {
            ...
            if (
              ...
              console.log(
                "[Call Component]  delete member because member refuse or timeout"
              );
              ...
              if (_this.$data.callType != 2) {
                ...
                _this.$message.error(
                  err[member.status] ? err[member.status] : "对方忙"
                );
              }
    ...
    update_members(key, status, val) {
      console.log(
        `[Call Component]  update_members key=${key}, status=${status}, val=${val}`
      );
      /*
       * key: 与会成员
    ...
    add_v_el() {
      ...
      console.log(
        "[Call Component]  add_v_el members",
        JSON.stringify(members)
      );
      for (const key in members) {
        let item = members[key];
        console.log("[Call Component]  add_v_el members key", key);
        let v_wrapper = document.querySelector(
          `.videos-wrapper .local[id="${key}"]`
        );
        console.log(
          `[Call Component]  add_v_el v_wrapper = ${v_wrapper}, item.el = ${item.el}, key=${key}`
        );
    ...
    send_invite_msg(tos) {
      ...
      tos.map((item) => {
        if (!this.$data.members[item]) {
          // 已经在会议中的不再邀请

          this.emedia.setConferenceAttrs({
            key: "invitee_" + item,
            val: JSON.stringify({ status: "calling" }),
          });
          ...
          let set_options = {
            ...
            success: function(id, serverMsgId) {
              console.log("[Call Component]  send invite success", {
                id,
                serverMsgId,
              });
            },
          ...
          console.log(
            "[Call Component]  send invite set_options",
            JSON.stringify(set_options)
          );
          msg.set(set_options);
    ...
    async invited_join(msg) {
      this.$data.confr_info = {
        confrId: msg.ext.confrId,
        password: msg.ext.confrPwd,
      };
      ...
      try {
        ...
        this.$data.wait_invite_cattr_timer = setTimeout(() => {
          ...
          if (this.$data.wait_invite_cattr_timeout) {
            console.log(
              "[Call Component]  hangup 5s 后还没收到 邀请自己的会议属性 退出会议"
            );
            this.hangup();
          }
        }, 5000);
    ...
    show_calling() {
      ...
      this.$data.wait_invite_cattr_timeout = false;
      clearTimeout(this.$data.wait_invite_cattr_timer);
      this.$data.call_status = "calling";

      if (this.$data.callType != 2) {
        if (this.$data.call_role == "callee" && this.$data.from)
      ...
      return new Promise((resolve, reject) => {
        _this.emedia
          ...
          .then((pushedStream) => {
            ...
            _this.emedia.streamBindVideo(
              _this.$data.pushedStream,
              _this.$refs.localVideo
            );
    ...
    sub_remotes() {
      ...
      for (let index = 0; index < _uids.length; index++) {
        ...
        let v_wrapper = document.querySelector(
          `.videos-wrapper .item-wrapper[id="${m.name}"]`
        );
    ...
    hangup() {
      // 多种情况会触发 挂断
      if (this.$data.callType != 2 && this.$data.call_role == "caller") {
    ...
    reset() {
      ...
      for (const key in this.$data) {
        if (key.indexOf("invitee_attr_timer") > -1)
          clearTimeout(this.$data[key]);
      }

      clearTimeout(this.$data.wait_invite_cattr_timer);

      Object.assign(this.$data, this.$options.data(), { os: this.$data.os }); // os 不重置
    ...
    controlAudio() {
      ...
      let _this = this;
      this.emedia[funName](pushedStream).then(() => {
