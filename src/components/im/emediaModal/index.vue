<template>
  <Draggable v-show="emediaModalVisible" id="drag1">
    <div
      v-bind:class="{
        rtcVoiceContent: streamType == '语音',
        rtcVideoContent: streamType == '视频'
      }"
    >
      <div v-if="callerWaitVisible" class="mask">
        正在等待{{ contact }}接受邀请
      </div>
      <div v-if="calleeWaitVisible" class="mask">
        {{ contact }}请求{{ streamType }}通话
      </div>
      <div v-if="voiceCallVisible" class="voiceCall">
        正在与{{ contact }}通话
      </div>
      <video
        v-show="streamType == '视频'"
        ref="localVideo"
      ...
      <i
        v-show="showMute"
        class="el-icon-turn-off-microphone font microphone"
        isopen="true"
        ref="audio"
        @click="controlStream('audioControl')"
      ></i>
      <i
        v-show="showAccept"
      ...
      <a-icon
        v-show="showMute && streamType == '视频'"
        class="font camera"
        ref="video"
        isopen="true"
        type="video-camera"
        @click="controlStream('videoControl')"
      />
      ...
      <i
        v-show="showMute && streamType == '视频'"
        class="el-icon-refresh font toggle"
        @click="toggleClick"
      ></i>
    </div>
  </Draggable>
...
export default {
  ...
  methods: {
    ...mapActions(["showMultiAVModal", "hideMultiAVModal"]),
    initWebRTC() {
      ...
      WebIM.call = new WebIM.WebRTC.Call({
        ...
        listener: {
          ...
          onError: function(e) {
            let close = false;
            const closeButton = document.getElementById("webrtc_close");
            if (e && e.message) {
              switch (e.message) {
            ...
            me.$message.error(
              e && e.message
                ? e.message
                : "An error occured when calling webrtc"
            );
            me.closeEmediaModal();
          },
    ...
    controlStream(type) {
      ...
      if (type === "audioControl") {
        ...
        this.$refs.audio.style.color = this.$refs.audio.isopen
          ? "#2c3e50"
          : "#4eb1f4";
        this.$refs.audio.isopen = !this.$refs.audio.isopen;
      } else {
        controlType = this.$refs.video.isopen ? 3 : 2;
        this.$refs.video.style.color = this.$refs.video.isopen
          ? "#2c3e50"
          : "#4eb1f4";
