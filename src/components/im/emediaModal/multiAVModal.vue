    ...
    <div class="multiAVModal" v-show="multiAVModalVisible">
      ...
      <div class="row-bg">
        ...
        <div
          v-for="(item, i) in rv"
          :key="i"
          class="grid-content bg-purple videoBox"
        >
          <video class="video" autoplay playsinline :ref="`rv_${i}`" />
          <div class="tools">
            <span>{{ rv[i].nickName }}</span>
            <i
              class="el-icon-video-camera red"
              @click="controlRemoteVideo(i)"
            ></i>
          </div>
        </div>
      </div>
      <el-row class="toolsBox">
        ...
        <el-col :span="3">
          <i
            :class="
              rv_local.openAudio
                ? 'el-icon-microphone toolBtn'
                : 'el-icon-microphone active'
            "
        ...
        <el-col :span="3">
          <i
            :class="
              rv_local.openVideo
                ? 'el-icon-video-camera toolBtn'
                : 'el-icon-video-camera active'
            "
      ...
      WebIM.EMService.onStreamAdded = function(member, stream) {
        ...
        if (located) {
          ...
          if (ifContain === false) {
            ...
            rvCount++;
            const str = "rv_" + index.toString();
            const video = me.$refs[str][0];
