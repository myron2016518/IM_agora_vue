...
export default {
  ...
  mounted() {
    ...
    console.log("======AgoraRTC222=====", AgoraRTC);
    const { callStatus } = this.$store.state.agora;
  ...
  methods: {
    ...
    controlStream(type) {
      if (type === "audioControl") {
        this.$refs.audio.isopen
          ? rtc.localAudioTrack.setEnabled(true)
          : rtc.localAudioTrack.setEnabled(false);
        this.muteBtnIcon = this.$refs.audio.isopen
          ? microphone
          : microphoneMute;

        this.$refs.audio.isopen = !this.$refs.audio.isopen;
      } else {
        this.$refs.video.isopen
