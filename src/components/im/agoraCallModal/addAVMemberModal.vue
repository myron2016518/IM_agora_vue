...
export default {
  ...
  computed: {
    ...
    videos() {
      const { joinedMembers, invitedMembers } = this.$store.state.agora;
      console.log(
        "joinedMembers>>",
        joinedMembers,
        "invitedMembers>>",
      ...
      const a = joinedMembers.concat(invitedMembers);
      console.log("a>>>>>>", a);
  ...
  watch: {
    onJoin() {
      const { callStatus, confr } = this.$store.state.agora;
  ...
  mounted() {
    ...
    const videos = joinedMembers.concat(invitedMembers);
    if (videos.length > 4) {
      this.span = 6;
    } else {
      this.span = 12;
  ...
  methods: {
    ...
    video_class(item) {
      const joining = item ? "" : "joining";
      const newClass = "default" + joining;
    ...
    addListener() {
      rtc.client.on("user-published", async (user, mediaType) => {
        ...
        const { joinedMembers } = this.$store.state.agora;
        let videoElm = "";
        let exist = false;
      ...
      rtc.client.on("volume-indicator", (result) => {
        const isTalting = [...this.$data.isTalting];
        result.forEach((volume, index) => {
          ...
          if (volume.level > 1 && !isTalting.includes(volume.uid)) {
            ...
            const i = isTalting.indexOf(volume.uid);
    ...
    async join() {
      ...
      const appId = "13ac9b352d9245a090ffff3c72099a83";
      const { confr } = this.$store.state.agora;
      const imUserName = WebIM.conn.context.jid.name;
      const params = {
        username: imUserName,
        channelName: confr.channel,
        appkey: WebIM.conn.appKey,
      };
      console.log("=====confr1=====", confr);
      ...
      const videoElm = "video" + WebIM.conn.context.jid.name;
    ...
    interval() {
      ...
      this.intervalID = setInterval(() => {
        ...
        const time = this.loadTime(hour, minute, second);
