...
const Group = {
  ...
  actions: {
    ...
    onGetGroupinfo: function(context, payload) {
      ...
      // TODO
      let gid = payload.selectId || payload.select_groupid;
      let options = {
        groupId: gid, //群组id
        success: function(resp) {
    ...
    onInviteGroup: function(context, payload) {
      const { selectId, selectName } = payload;
      let option = {
        users: [selectName],
        groupId: selectId,
    ...
    onUpdataGroupInfo: function(context, payload) {
      const { selectId, updateName, updateDesc } = payload;
      let option = {
        groupId: selectId,
        ...
        success: function(resp) {
          this._vm.dispatch("onGetGroupUserList");
          this._vm.dispatch("onGetGroupinfo", { selectId });
          this.$forceUpdate();
        }
      };
    ...
    onSetAdmin: function(context, payload) {
      const { selectId, selectName } = payload;
      let options = {
        groupId: selectId, // 群组id
        username: selectName, // 用户名
        success: function(resp) {
    ...
    onAddGroupBlack: function(context, payload) {
      ...
      let options = {
        groupId: selectId, // 群组ID
        username: selectName, // 将要被加入黑名单的用户名
        success: function(resp) {
          this._vm.dispatch("onGetGroupinfo", { selectId });
          this.$forceUpdate();
    ...
    onRemoveGroupBlack: function(context, payload) {
      const { selectId, removeGroupName } = payload;
      var options = {
        groupId: selectId, // 群组ID
        username: removeGroupName, // 需要移除的用户名
        success: function(resp) {
          console.log("移除成功Response: ", resp);
          this._vm.dispatch("onGetGroupBlack", { selectId });
          this.$forceUpdate();
        },
        error: function(e) {}
      };
    ...
    onGetGroupBlack: function(context, payload) {
      let selectId = payload.groupid || payload.selectId;
      let option = {
        groupId: selectId,
