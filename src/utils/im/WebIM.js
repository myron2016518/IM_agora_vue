...
WebIM.conn.listen({
  onOpened: function(message) {
    ...
    store.commit("onGetFirendBlack");
    store.commit("onGetContactUserList");
  ...
  onTextMessage: function(message) {
    ...
    store.commit("noticeCall", message); // 通知给通话组件，是否别人邀请通话
    ...
    if (message.ext && message.ext.action === "invite") {
      ...
      const callVideo = store.getters.getAgora;
      ...
      if (callVideo.callStatus > CALLSTATUS.idle) {
        ...
        if (message.ext.callId == callVideo.callId) {
          // 多人会议中邀请别人
          store.dispatch("sendAlerting", {
            to: from,
            calleeDevId: callerDevId,
            callId
          }); // 回复alerting消息
          store.dispatch("setCallStatus", CALLSTATUS.alerting); // 更改为alerting状态
        } else {
          return store.dispatch("answerCall", {
            result: "busy",
      ...
      store.dispatch("updateConfr", message);
      store.dispatch("sendAlerting", {
        to: from,
        calleeDevId: callerDevId,
        callId
      ...
      switch (msgInfo.action) {
        ...
        case "confirmRing":
          ...
          if (
            ...
            store.dispatch("setCallStatus", CALLSTATUS.idle);
            return;
          }
          deviceId = msgInfo.calleeDevId;
          store.dispatch("setCallStatus", CALLSTATUS.receivedConfirmRing);
          console.log("清除定时器2");
        ...
        case "confirmCallee":
          ...
          store.dispatch("setCallStatus", CALLSTATUS.confirmCallee);
          break;
        case "cancelCall":
  ...
  onPresence: function(message) {
    ...
    const selectId = store.state.group.groupInfo.gid; // 群组相关操作，更新数据时需要
    switch (message.type) {
      case "subscribe": {
        const options = {
          isShow: false,
        ...
        store.dispatch("acceptSubscribe", message.from); //自动订阅
      ...
      case "memberJoinPublicGroupSuccess": // 成员加入聊天室成功回调
        store.dispatch("onGetGroupinfo", { selectId });
        Message({
          type: "success",
